...
function LoadToast({ toast, onDismiss }: LoadToastProps) {
	return (
		<div
			...
			aria-atomic="true"
		>
			<span aria-hidden="true">{toast.kind === "success" ? "✅" : "⚠️"}</span>
...
function EditorView() {
	...
	const [syncTasksToast, setSyncTasksToast] = useState<{
		kind: "success" | "error";
		message: string;
	} | null>(null);
	...
	async function handleSyncTasks() {
		...
		try {
			...
			if (result.errors.length === 0) {
				setSyncTasksToast({
					kind: "success",
					message: `${result.updated} agent(s) updated`,
				});
	...
	const selectedAgent =
		project?.agents.find((a) => a.id === selectedAgentId) ?? null;
		...
		<div className="editor-view">
			...
			<div className="editor-view__main">
				...
				<aside className="editor-view__sidebar" aria-label="Agents">
					...
					{syncTasksToast && (
						<div
							...
							aria-live="polite"
						>
							{syncTasksToast.kind === "success" ? "✅" : "⚠️"}{" "}
							{syncTasksToast.message}
						</div>
					)}
					...
					{flowAgents.length > 0 ? (
						...
						<div className="editor-view__sidebar-empty">
							...
							<p>
								No agents yet.
								<br />
								Click <strong>+ New agent</strong> to start.
							</p>
						</div>
					)}
				</aside>
				...
				<div className="editor-view__canvas-area">
					<section
						className="editor-view__canvas-section"
						...
						{selectedAgent ? (
							...
							<div className="editor-view__agent-detail">
								<header className="editor-view__detail-header">
									<h2 className="editor-view__detail-name">
										{selectedAgent.name}
									</h2>
								...
								<dl className="editor-view__detail-meta">
									...
									<dt>Description</dt>
									<dd>
										{selectedAgent.description || <em>No description</em>}
									</dd>
									<dt>Profile</dt>
									<dd>
										<code>{selectedAgent.profilePath}</code>
									</dd>
									<dt>Metadata</dt>
									<dd>
										<code>{selectedAgent.adataPath}</code>
									</dd>
								</dl>
									...
									<section className="editor-view__detail-section">
										...
										<ul className="editor-view__detail-list">
											{selectedAgent.aspects.map((a) => (
												<li
													key={a.id}
													className={`editor-view__detail-item ${a.enabled ? "" : "editor-view__detail-item--disabled"}`}
												>
													<span className="editor-view__detail-item-name">
														{a.name}
													</span>
													<code className="editor-view__detail-item-path">
														{a.filePath}
													</code>
									...
									<section className="editor-view__detail-section">
										...
										<ul className="editor-view__detail-list">
											{selectedAgent.skills.map((s) => (
												<li
													key={s.id}
													className={`editor-view__detail-item ${s.enabled ? "" : "editor-view__detail-item--disabled"}`}
												>
													<span className="editor-view__detail-item-name">
														{s.name}
													</span>
										...
										<ul className="editor-view__detail-list">
											{selectedAgent.subagents.map((sub) => (
												<li key={sub.id} className="editor-view__detail-item">
													...
													<span className="editor-view__detail-item-desc">
														{sub.description}
													</span>
												</li>
											))}
...
export function App() {
	...
	const permissionsModalTarget = useAgentFlowStore(
		(s) => s.permissionsModalTarget,
	);
	const closePermissionsModal = useAgentFlowStore(
		(s) => s.closePermissionsModal,
	);
	...
	useEffect(() => {
		...
		try {
			...
			setLoadToast({
				kind: "error",
				message: `Failed to reconstruct canvas: ${message}`,
			});
		}
