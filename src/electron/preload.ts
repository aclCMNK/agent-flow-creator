...
const bridge: AgentsFlowBridge = {
  ...
  onSkillConflict(callback: (prompt: ExportSkillsConflictPrompt) => void) {
    ...
    ipcRenderer.on(IPC_CHANNELS.SKILL_CONFLICT_PROMPT, (_event, prompt: ExportSkillsConflictPrompt) => {
      callback(prompt);
    });
  },
  ...
  onProfileConflict(callback: (prompt: ExportProfileConflictPrompt) => void) {
    ...
    ipcRenderer.on(IPC_CHANNELS.PROFILE_CONFLICT_PROMPT, (_event, prompt: ExportProfileConflictPrompt) => {
      callback(prompt);
    });
  },
...
console.log(
  ...
  typeof (globalThis as Record<string, unknown>)["folderExplorer"] !== "undefined"
